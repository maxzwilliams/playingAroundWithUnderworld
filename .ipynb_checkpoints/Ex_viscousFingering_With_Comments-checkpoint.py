...
# The Darcy equation relates the gradients in pressure to the flux of material (velocity). Physically, then, the velocity (vector) is proportional to the spatial derivative of the pressure. Using a diffusion model as the constitutive model with a diffusion constant of $\frac{\kappa}{\mu_c}$ gives exactly our Darcy flow equation. We specify the diffusion constant later in the code.

darcy.constitutive_model = uw.systems.constitutive_models.DiffusionModel(mesh.dim)

darcy.constitutive_model
...
## setup an advection diffusion solver on the swarm. We give this a term u_Star_fun to keep track of
...
adv_diff.constitutive_model

# We have the advection diffusion equation for a concentration $c$ being advected by a velocity field $v_{sol}$ with and diffusing with a diffusion constant of $k$. The consitutive equation for the relation between the velocity and concentration is:
# \begin{equation}
#     u = k \frac{d c}{d x}
# \end{equation}
# This is of the form of the diffusionModel with a diffusion constant $k$ (we set this constant later).
...
# In the three cells bellow we define the viscousity and from it define the diffusivity in our darcy consitutive equation.
...
# Here we set the diffusivity in the consitutive equation for the advection-diffusion solver
